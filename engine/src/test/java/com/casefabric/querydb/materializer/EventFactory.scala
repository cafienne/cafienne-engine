...
import com.casefabric.cmmn.actorapi.event._
import com.casefabric.cmmn.actorapi.event.file.{CaseFileItemCreated, CaseFileItemTransitioned}
...
class EventFactory(actorId: String, caseDefinition: CaseDefinition, user: TenantUser) {
  ...
  def createCaseDefinitionApplied(user: TenantUser = user) : CaseDefinitionApplied = {
    val json = new ValueMap(
      ...
      ,Fields.engineVersion, CaseFabric.version.json
