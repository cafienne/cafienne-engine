...
trait ConsentGroupTables extends QueryDBSchema {
  ...
  final class ConsentGroupTable(tag: Tag) extends CaseFabricTable[ConsentGroupRecord](tag, "consentgroup") {
    // Columns
  ...
  class ConsentGroupMemberTable(tag: Tag) extends CaseFabricTable[ConsentGroupMemberRecord](tag, "consentgroup_member") {
    // Columns
