...
public class ModelActorTransaction {
    ...
    void perform() {
        if (message.getUser() == null) {
            throw new RuntimeException("Created a " + message +" without a user");
        }
        actor.addDebugInfo(() -> "---------- User " + message.getUser().id() + " in " + actor + " receives message " + message.getDescription(), message.rawJson());

//        if (message instanceof ActorInteractionMessage actorMessage) {
//            backOffice.handleActorCommunication(actorMessage);
//        } else
