...
class V1_1_36__ChangeTimerTablesAddRootCaseId(tablePrefix: String) extends ClassicEventDBSchemaScript {
  ...
  override def sql: String = {
    s"""ALTER TABLE public.${tablePrefix}timer ADD COLUMN root_case_id varchar(255) NOT null;
       |CREATE INDEX ${tablePrefix}timer_rootcaseid_idx on public.${tablePrefix}timer (root_case_id);
       | """.stripMargin
  }
}
