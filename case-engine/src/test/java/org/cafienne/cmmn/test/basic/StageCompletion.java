...
public class StageCompletion {
    ...
    public void testStage1Completion() {
        ...
        testCase.addTestStep(new StartCase(testUser, caseInstanceId, definitions, null, null), action -> {
        ...
        testCase.addTestStep(new MakePlanItemTransition(testUser, caseInstanceId, null, Transition.Occur, "TriggerStage1"), action -> {
            CaseAssertion casePlan = new CaseAssertion(action);
        ...
        // Trying to complete the stage should not change anything
        testCase.addTestStep(new MakePlanItemTransition(testUser, caseInstanceId, null, Transition.Complete, "Stage1"), action -> {
            CaseAssertion casePlan = new CaseAssertion(action);
            casePlan.assertStage("Stage1").assertLastTransition(Transition.Start, State.Active, State.Available);
            casePlan.assertStage("Stage1").assertPlanItem("Task1").assertState(State.Active);
            casePlan.assertStage("Stage1").assertPlanItem("Task2").assertState(State.Available);
    ...
    public void testStage2Completion() {
        ...
        // Trying to complete the stage should not change anything
        testCase.addTestStep(new MakePlanItemTransition(testUser, caseInstanceId, null, Transition.Occur, "TriggerStage2"), action -> {
            CaseAssertion casePlan = new CaseAssertion(action);
        ...
        testCase.addTestStep(new MakePlanItemTransition(testUser, caseInstanceId, null, Transition.Complete, "Task3"), action -> {
            CaseAssertion casePlan = new CaseAssertion(action);
            casePlan.assertStage(mainStageName).assertLastTransition(Transition.Start, State.Active, State.Available);
        ...
        testCase.addTestStep(new MakePlanItemTransition(testUser, caseInstanceId, null, Transition.Complete, mainStageName), action -> {
            CaseAssertion casePlan = new CaseAssertion(action);
    ...
    public void testStage3Completion() {
        ...
        // Trying to complete the stage should not change anything
        testCase.addTestStep(new MakePlanItemTransition(testUser, caseInstanceId, null, Transition.Occur, "TriggerStage3"), action -> {
            CaseAssertion casePlan = new CaseAssertion(action);
            casePlan.assertStage(mainStageName).assertLastTransition(Transition.Start, State.Active, State.Available);
        ...
        testCase.addTestStep(new MakePlanItemTransition(testUser, caseInstanceId, null, Transition.Complete, "Task5"), action -> {
            CaseAssertion casePlan = new CaseAssertion(action);
            casePlan.assertStage(mainStageName).assertLastTransition(Transition.Complete, State.Completed, State.Active);
