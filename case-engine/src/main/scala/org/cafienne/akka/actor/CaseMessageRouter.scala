...
object ClusteredRouterService extends MessageRouterService {
  ...
  def apply(system: ActorSystem) = {
    caseShardRouter = Some(ClusterSharding(system).shardRegion(CaseSystem.caseShardTypeName))
    processShardRouter = Some(ClusterSharding(system).shardRegion(CaseSystem.processShardTypeName))
    tenantShardRouter = Some(ClusterSharding(system).shardRegion(CaseSystem.tenantShardTypeName))
    this
  }
}
