 ...
 */
@Manifest
public class CreateCaseFile extends CaseFileOperation {
    private static final CaseFileItemTransition transition = CaseFileItemTransition.Create;
     ...
     */
    public CreateCaseFile(TenantUser tenantUser, String caseInstanceId, Value<?> newContent) {
        super(tenantUser, caseInstanceId, newContent, transition);
    }

    public CreateCaseFile(ValueMap json) {
        super(json, transition);
    }

    @Override
    void apply(Case caseInstance, CaseFile caseFile, Value<?> content) {
        caseFile.createContent(content);
    }
}
