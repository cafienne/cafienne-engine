...
@Manifest
case class UpdateColumnDefinition(val user: BoardUser, val columnId: String, val title: Option[String], val form: Option[ValueMap]) extends BoardDefinitionCommand(user) with UpdateDefinition {
  /**
