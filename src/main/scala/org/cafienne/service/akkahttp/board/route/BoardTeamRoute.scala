...
class BoardTeamRoute(override val caseSystem: CaseSystem) extends BoardRoute {
  ...
  @RequestBody(description = "User information", required = true, content = Array(new Content(schema = new Schema(implementation = classOf[BoardTeamAPI.BoardTeamFormat]))))
