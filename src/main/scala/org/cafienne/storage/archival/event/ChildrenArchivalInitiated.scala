...
@Manifest
case class ChildrenArchivalInitiated(metadata: ActorMetadata, members: Seq[ActorMetadata], override val optionalJson: Option[ValueMap] = None) extends ArchivalEvent {
...
object ChildrenArchivalInitiated {
  def deserialize(json: ValueMap): ChildrenArchivalInitiated = {
    ...
    ChildrenArchivalInitiated(metadata, members, Some(json))
  }
}
