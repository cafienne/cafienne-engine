...
class TaskTransaction(taskId: String, persistence: RecordsPersistence)(implicit val executionContext: ExecutionContext) extends LazyLogging {
  ...
  def handleEvent(evt: CaseEvent): Future[Done] = {
    ...
    evt match {
      // Events of type HumanTaskCreated are no longer generated by engine. Instead, HumanTaskActivated now carries all info.
      // However, for compatibility we still handle the old version of the event, because the old HumanTaskActivated
      // does not carry sufficient information. It really needs the HumanTaskCreated event info (specifically createdOn and createdBy info).
      // Fortunately, both events were _always_ generated in the _same_ transaction, and we depend on that in the below logic.
