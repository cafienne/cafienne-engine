...
@Path("/platform")
class PlatformRoute(platformQueries: PlatformQueries)(override implicit val userCache: IdentityProvider, override implicit val caseSystem: CaseSystem) extends TenantRoute {
        ...
        pathEndOrSingleSlash {
          ...
          entity(as[TenantAPI.PlatformUsersUpdateFormat]) { request =>
            ...
            val queries = for {
              tenantsByUser <- platformQueries.whereUsedInTenants(existingUserIds, request.tenants)
              casesByUser <- platformQueries.whereUsedInCases(existingUserIds, request.tenants)
              tenantsByNewUser <- platformQueries.whereUsedInTenants(newUserIds, request.tenants)
              casesByNewUser <- platformQueries.whereUsedInCases(newUserIds, request.tenants)
