...
import org.cafienne.service.api.tenant.{TenantRecord, TenantOwnerRecord, UserQueries, UserRoleRecord}
...
class TenantTransaction(tenant: String, userQueries: UserQueries, updater: RecordsPersistence)(implicit val executionContext: ExecutionContext) extends LazyLogging {
  ...
  val tenantOwners = scala.collection.mutable.HashMap[String, TenantOwnerRecord]()
  val rolesByUserAndRoleName = scala.collection.mutable.HashMap[(String, String), UserRoleRecord]()
...
//    println("Modified: "+rolesByUserAndRoleName.keySet)
  ...
  private def storeRole(role: UserRoleRecord) = {
...
//    println("Storing role "+key+": "+role)
  ...
  def handlePlatformEvent(event: PlatformEvent): Future[Done] = {
    event match {
      case newTenant: TenantCreated => {
        tenants.put(newTenant.tenantName, TenantRecord(newTenant.tenantName()))
      ...
      case disabledTenant: TenantDisabled => {
        tenants.put(disabledTenant.tenantName(), TenantRecord(disabledTenant.tenantName(), false))
        // now execute a sql that disables all tenant users?
      }
      case enabledTenant: TenantEnabled => {
        tenants.put(enabledTenant.tenantName(), TenantRecord(enabledTenant.tenantName(), true))
  ...
  def addTenantOwner(event: OwnerAdded) = {
    tenantOwners.put(event.userId, TenantOwnerRecord(event.getActorId, event.userId))
    Future.successful(Done)
  }

  def removeTenantOwner(event: OwnerRemoved) = {
    tenantOwners.put(event.userId, TenantOwnerRecord(event.getActorId, event.userId, false))
    Future.successful(Done)
  }

  def createTenantUser(event: TenantUserCreated): Future[Done] = {
    // Create a user with an empty role
    storeRole(UserRoleRecord(event.userId, tenant, role_name = "", name = event.name, email = event.email, enabled = true))
  }

  def disableTenantUser(event: TenantUserDisabled): Future[Done] = {
    // This should not erase the name of the user
    storeRole(UserRoleRecord(event.userId, tenant, role_name = "", name = "", email = "", enabled = false))
  }

  def enableTenantUser(event: TenantUserEnabled): Future[Done] = {
    // This should not erase the name of the user
    storeRole(UserRoleRecord(event.userId, tenant, role_name = "", name = "", email = "", enabled = true))
  }
  ...
  def addUserRole(event: TenantUserRoleAdded): Future[Done] = {
    ...
    storeRole(UserRoleRecord(event.userId, tenant, role_name = event.role, name = "", email = "", enabled = true))
  }

  def removeUserRole(event: TenantUserRoleRemoved): Future[Done] = {
    // Also add all roles the user has within this tenant.
    storeRole(UserRoleRecord(event.userId, tenant, role_name = event.role, name = "", email = "", enabled = false))
  }
